...
void loop(){
  for(int first_line=0; first_line<16; first_line++){     // 첫번째 줄
    if(first_line == 0){        // 프로그램 처음 실행 하기 위한 조건문
      
      lcd.setCursor(0, 0);  // 커서 위치 0,0    첫번째 줄
      lcd.write(byte(1));       // 팩맨 얼굴 출력
 
      lcd.setCursor(1, 0);    // 커서 위치 1,0    첫번째 줄
      lcd.print("Hello, Eduino!!");// Hello, Eduino!! 출력 실제로 값을 수정 (1번째 줄 : co2 농도 값 등등 )        
            
      lcd.setCursor(0, 1);    // 커서 위치 0,1    두번째 줄
      lcd.print("Have a nice day!");  // Have a nice day! 출력 실제로 값을 수정 (2번째 줄 : co2 농도 값 등등)
    }
    else {
